...
(function() {
    ...
    ImboClient.prototype.deleteImageByIdentifier = function(imageIdentifier, callback) {
        ...
        this.request('DELETE', url, null, function(err, res) {
            console.log(err, res);
        });
    };
    ...
    ImboClient.prototype.addImageFromBlob = function(blob, callback) {
        ...
        this.getImageIdentifierFromString(blob, function(err, imageIdentifier) {
            if (err) {
                return setImmediate(callbacks.onComplete, err);
            }

            var url = this.getSignedResourceUrl('PUT', this.getResourceUrl(imageIdentifier));

            // Set up PUT-request
            this.request('PUT', url, blob, callbacks);
        }.bind(this));
    };
    ...
    ImboClient.prototype.addImage = function(path, callback) {
        this.getFileContents(path, function(err, data) {
            if (err) {
                return setImmediate(callback, err);
            }

            this.addImageFromBlob(data, callback);
        }.bind(this));
    };

    ImboClient.prototype.addImageFromUrl = function(url, callback) {
        this.getUrlContents(url, function(err, data) {
            if (err) {
                return setImmediate(callback, err);
            }
    ...
    ImboClient.prototype.getUserInfo = function(callback) {
        this.request('GET', this.getUserUrl(), null, function(err, res) {
            console.log(err, res);
        });
    };
    ...
    ImboClient.prototype.getImages = function(query, callback) {
        ...
        // Fetch the response
        this.request('GET', url, null, function(err, res) {
            console.log(err, res);
        });
    };
    ...
    ImboClient.prototype.editMetadata = function(imageIdentifier, data, callback) {
        ...
        this.request('POST', url, meta, function(err, res) {
            console.log(err, res);
        });
    };
    ...
    ImboClient.prototype.replaceMetadata = function(imageIdentifier, data, callback) {
        ...
        var meta = typeof data === 'string' ? data : JSON.stringify(data);
        this.request('PUT', url, meta, function(err, res) {
            console.log(err, res);
        });
    };
